 ...
 * - runtime loads HLS from CDN to avoid bundler errors
 * - episode picker modal before playing
 * - download panel with copy-link & "try direct" download attempt
 * - square images, inline components only
 *
...
export default function AnimePage(): JSX.Element {
  ...
  // direct-download attempt state
  const [directDownloading, setDirectDownloading] = useState<{ url: string; status: string } | null>(null);
  ...
  // load HLS script from CDN if needed
  ...
  // NEW: when user clicks Watch, fetch episodes and show episode picker
  async function onWatchClick(series: SeriesItem) {
    setWatchError(null);
    setVideoSrc(null);
    setVideoTitle(null);
    setEpisodes(null);
    setEpisodePickerFor(series.title ?? "Series");
    ...
    try {
      ...
      setEpisodes(eps);
      // do not auto-play; user will choose episode
    } catch (err) {
      const msg = err && (err as any).message ? (err as any).message : String(err);
      setWatchError(msg);
      logDebug("Episode list error: " + msg);
      // fallback to attempting immediate watch flow using original behavior
      try {
        // attempt old behavior: pick first episode and play
        if (series.link) {
          const eps = await fetchEpisodes(series.link);
          const ep = eps[0];
          if (ep?.link) {
            await watchEpisode(series, ep);
          }
        }
      } catch (e) {
        logDebug("Fallback watch attempt failed: " + String(e));
      }
    } finally {
  ...
  async function watchEpisode(series: SeriesItem, ep: EpisodeItem) {
    ...
    try {
      ...
      // If m3u8, attempt to use runtime-loaded hls.js
      if (/\.m3u8(\?|$)/i.test(candidate)) {
        try {
          ...
          if (Hls && videoRef.current) {
            ...
            // close episode picker if open
            setEpisodes(null);
            setEpisodePickerFor(null);
            return;
          }
        } catch (e) {
          logDebug("hls runtime load failed, fallback to native or download");
        }
      }
      ...
      try {
        ...
        logDebug("Fallback download error: " + String(e));
      }
      setIsWatching(false);
    } finally {
      setWatchLoading(false);
  ...
  // try to fetch the asset and trigger in-page download (may fail due to CORS)
  async function tryDirectDownload(url: string) {
    setDirectDownloading({ url, status: "starting" });
    try {
      setDirectDownloading({ url, status: "fetching" });
      ...
      a.click();
      a.remove();
      setDirectDownloading({ url, status: "done" });
      setTimeout(() => setDirectDownloading(null), 1500);
    } catch (e) {
      const msg = e && (e as any).message ? (e as any).message : String(e);
      setDirectDownloading({ url, status: "failed: " + msg });
      // do not throw — user can still open in new tab
    }
  }
  ...
  function copyToClipboard(text: string) {
    try {
      ...
      logDebug("Clipboard failed: " + String(e));
    }
  }
      ...
      <section className="max-w-5xl mx-auto">
        <header className="mb-6 text-center">
          ...
          <p className="mt-2 text-slate-300 max-w-2xl mx-auto">
            Search Anichin, pick episodes, watch inline (HLS support), or grab direct download hosts. Cards animate into view
            as you scroll.
          </p>
        </header>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-3xl bg-[#071227] rounded-xl shadow-lg p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div>
                  <button onClick={() => { setEpisodes(null); setEpisodePickerFor(null); }} className="text-slate-300">Close</button>
                </div>
              </div>
              ...
              <div className="max-h-80 overflow-auto space-y-2">
                {episodes.map((ep, idx) => (
                  <div key={idx} className="p-3 bg-white/3 rounded-lg flex items-center justify-between">
                    <div>
                      <div className="font-medium">{ep.title || `Episode ${ep.episodeNumber || idx+1}`}</div>
                      <div className="text-xs text-slate-400">{ep.subStatus || ""} • {ep.releaseDate || ""}</div>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          // find series from results using episodePickerFor title
                          const series = results.find((r) => r.title === episodePickerFor) || ({} as SeriesItem);
                          watchEpisode(series, ep);
                        }}
                      ...
                      <button
                        onClick={async () => {
                          // fetch download links for selected ep and show download panel
                          setDownloadError(null);
                          setDownloadLinks(null);
                          ...
                          try {
                            ...
                            setDownloadError((err as any)?.message || String(err));
                          } finally {
                            setDownloadLoading(false);
                          }
                        }}
              ...
              {downloadLinks.map((g, idx) => (
                <div key={idx} className="p-3 border border-white/6 rounded-lg bg-black/20">
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-sm font-medium">{g.resolution || "Unknown resolution"}</div>
                    <div className="text-xs text-slate-400">Hosts: {g.hosts.length}</div>
                  </div>
                    ...
                    {g.hosts.map((h, j) => {
                      ...
                      return (
                        <div key={j} className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => tryDirectDownload(url)}
                            className="px-2 py-1 bg-white/6 rounded text-xs"
                            title="Attempt direct download (may fail due to CORS)"
                          >
                            {directDownloading?.url === url && directDownloading.status !== "done" ? (
                              <span className="inline-flex items-center gap-2"><Spinner /> <span className="text-xs">{directDownloading.status}</span></span>
                            ) : (
                              <span className="text-xs">Try Direct</span>
                            )}
                          </button>
                        </div>
                      );
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-4xl bg-[#071227] rounded-xl shadow-lg p-4">
              ...
              <div className="mt-3 text-xs text-slate-400">
                Playback attempts HLS (m3u8) first using hls.js loaded at runtime. If unavailable, the best host is shown
