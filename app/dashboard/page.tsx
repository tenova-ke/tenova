...
export default function DashboardPage(): JSX.Element {
  ...
  const tools = [
    ...
    { id: "webtools", title: "Web Tools", icon: <Globe className="w-6 h-6" /> },
    { id: "encrypt", title: "Encrypt", icon: <Lock className="w-6 h-6" /> },
    { id: "downloads", title: "Downloads", icon: <Download className="w-6 h-6" /> },
  ];

  // Stats & auto-data (typed)
  const [monthSeries, setMonthSeries] = useState<Point[]>(
    () => genInitialSeries(20, 1200, 600)
  );
  const [weekSeries, setWeekSeries] = useState<Point[]>(
    () => genInitialSeries(14, 320, 150)
  );
  const [todaySeries, setTodaySeries] = useState<Point[]>(
    () => genInitialSeries(12, 48, 22)
  );
  const [apiSeries, setApiSeries] = useState<Point[]>(
    () => genInitialSeries(20, 380, 40)
  );
  const [pingHistory, setPingHistory] = useState<Point[]>(
    () => genInitialSeries(18, 60, 50)
  );
  ...
  useEffect(() => {
    const tick = async () => {
      ...
      setUptimeStatus(Math.random() >= 0.03);
    };
  ...
  // floating animation base (framer)
  const floatVariants = {
    rest: { y: 0 },
    float: { y: -8 },
  };
    ...
    <main className="min-h-screen p-6 bg-gradient-to-br from-[#0b0512] via-[#14102a] to-[#071021] text-white">
      ...
      <section className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <aside className="space-y-4">
          ...
          {/* Uptime + Ping card */}
          <div
            className={`p-4 rounded-2xl border ${uptimeStatus ? "border-green-400" : "border-red-500"} bg-white/4`}
          >
            <div className="flex items-center justify-between">
              <div>
