...
/**
 * Dashboard page (full)
 * - Put in app/dashboard/page.tsx
 * - Requires: tailwindcss, framer-motion, recharts, lucide-react
 *
 * Notes:
 * - The health check used for real pinging is PING_URL (default "/api/health").
 *   Change it to your real health endpoint if different.
 * - Clicking tool tiles does a router.push to internal pages (no popup).
 */
...
export default function DashboardPage(): JSX.Element {
  ...
  const [uptimeStatus, setUptimeStatus] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    const doPing = async () => {
      // use AbortController so we time out
      const ac = new AbortController();
      const timeout = setTimeout(() => ac.abort(), 4000); // 4s timeout

      const start = performance.now();
      try {
        const resp = await fetch(`${PING_URL}?_=${Date.now()}`, {
          method: "GET",
          signal: ac.signal,
          cache: "no-store",
        });
    ...
    <main className="min-h-screen p-6 bg-gradient-to-br from-[#0b0512] via-[#14102a] to-[#071021] text-white">
        ...
        <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          >
            <button
              onClick={() => {
                setQuery("");
                // route to a full tools page if you have one, else just clear filter
                router.push("/tools");
              }}
        ...
        <aside className="space-y-4">
          ...
          <div className={`p-4 rounded-2xl border ${uptimeStatus ? "border-green-400" : "border-red-500"} bg-white/4`}>
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                onClick={() => {
                  // manual ping now (trigger by altering a key)
                  // quick manual ping by toggling showGraphFor to force immediate UI update
                  (async () => {
                    try {
                      await fetch(`${PING_URL}?_=${Date.now()}`, { cache: "no-store" });
                    } catch (e) {
                      // ignore
                    }
                  })();
      ...
      <section className="max-w-6xl mx-auto mt-8 p-6 rounded-2xl bg-gradient-to-r from-white/2 to-white/3 border border-white/6">
        ...
        <ol className="text-sm text-gray-300 space-y-2 list-decimal list-inside">
          ...
          <li>The ping card checks <code>{PING_URL}</code> every 5 seconds to keep the site alive and measure latency.</li>
