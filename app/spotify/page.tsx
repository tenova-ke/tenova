...
export default function SpotifyPage() {
  ...
  async function doSearch(q: string) {
    ...
    setTracks([]);
    try {
      // 🔎 Prexzy Spotify Search
      const res = await fetch(
        `https://apis.prexzyvilla.site/search/spotify?query=${encodeURIComponent(q)}`
      );
  ...
  function extractDownloadUrl(payload: any): string | undefined {
    // Gifted API result format
    return payload?.result?.download_url;
  }
  ...
  async function playTrack(idx: number) {
    ...
    try {
      // 🎶 Gifted SpotifyDl
      const res = await fetch(
        `https://api.giftedtech.web.id/api/download/spotifydl?apikey=gifted&url=${encodeURIComponent(t.link)}`
      );
      const data = await res.json();
  ...
  async function downloadTrack(track: Track) {
    ...
    try {
      // 🎶 Gifted SpotifyDl
      const res = await fetch(
        `https://api.giftedtech.web.id/api/download/spotifydl?apikey=gifted&url=${encodeURIComponent(track.link)}`
      );
      const data = await res.json();
      const fileUrl = extractDownloadUrl(data);
      if (!fileUrl) {
