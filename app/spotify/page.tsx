...
export default function SpotifyPage() {
  ...
  async function doSearch(q: string) {
    ...
    try {
      const res = await fetch(
        `${GIFTED_BASE}/api/search/spotifysearch?apikey=${GIFTED_KEY}&query=${encodeURIComponent(q)}`
      );
      ...
      const normalized = items.map((t: any) => ({
        ...
        artists: t.artist,
        link: t.url,
        image: null, // will enrich later if needed
        duration: t.duration,
        id: t.id,
      }));
  ...
  async function fetchDownloadUrl(spotifyUrl: string) {
    const url = `${GIFTED_BASE}/api/download/spotifydl?apikey=${GIFTED_KEY}&url=${encodeURIComponent(
      spotifyUrl
    )}`;
