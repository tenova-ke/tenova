...
export default function SupportLearningPage() {
  ...
  const wallet = useCount(74200); // KES or unit you choose
    ...
    <main className="min-h-screen bg-gradient-to-br from-black via-slate-900 to-zinc-900 text-white p-8">
      ...
      <section className="max-w-6xl mx-auto grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {LANGS.map((lang, i) => (
          <motion.article
            key={lang.id}
            className={`relative group overflow-hidden rounded-2xl p-5 border border-white/6 shadow-2xl`}
              ...
              <div>
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${lang.colorFrom} ${lang.colorTo} flex items-center justify-center shadow-lg`}>
                    <CodeIcon className="w-6 h-6 opacity-90" />
                  </div>
      ...
      <style jsx>{`
        /* gentle floating background badge */
        .floating-badge {
          position: absolute;
          right: -30px;
          top: -24px;
...
/* tiny helpers inside same file so it's copy-paste friendly */
function UsersIconPlaceholder() {
  return <svg width="18" height="18" className="text-emerald-300" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M17 21v-2a4 4 0 00-4-4H7a4 4 0 00-4 4v2"/></svg>;
}

function FloatingBadge({ colorFrom, colorTo }: { colorFrom: string; colorTo: string }) {
  // Tailwind gradient classes passed in from LANGS entries
  return (
    <div
...
function WHA(text = "") {
  return `https://wa.me/254758476795?text=${encodeURIComponent(text)}`;
}
